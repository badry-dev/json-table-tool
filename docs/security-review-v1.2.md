...
- Keep `memory://` only for the **single-worker, single-instance** default. Because counters are per worker, any deployment running more than one worker or instance must set a shared `RATELIMIT_STORAGE_URI` (`redis://...`) or it is not enforcing the configured limit — README's `--workers 4` example and roadmap 2.8's `--workers 2` are both affected. Add a startup check that warns (or fails fast under `APP_ENV=production`) when the worker count is > 1 while storage is `memory://`, correct the deployment examples, and add a test asserting the per-worker semantics explicitly rather than leaving them implied (roadmap 2.10).
