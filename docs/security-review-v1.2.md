...
**Effort:** 1-2h. **Tests:** mock `getaddrinfo` to raise/sleep and assert the bounded *caller* wait (`Future.result`) and the admission error under saturation — never a bound on teardown, which nothing here enforces; port rejection tests.
