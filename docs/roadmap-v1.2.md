...
- **D2.** Delete `find_candidate_arrays` + its tests, or repurpose it to pre-fill "recommended paths" in the tree picker? (Recommendation: delete; it's superseded. P10/F8)
...
**Acceptance:** `pip-audit -r requirements.txt` = 0 vulnerabilities; CI green; all 82 tests pass after upgrades; `pip install -r requirements.txt` no longer installs pytest.

---

### Phase 1 — Security Hardening  (≈ 1.5-2 days)

**Goal:** close every open security finding except those explicitly deferred (F15 pending D4-adjacent decision).

| # | Task | Files | Fixes |
|---|---|---|---|
| 1.1 | **Formula-injection sanitization**: `sanitize_cell(value)` in `helpers.py` (prefix `'` when value starts with `= + - @` or tab/CR); apply in `export_csv`, `export_xlsx` (replace duplicated `isinstance(v,(dict,list))` branches), and in `app.js` `downloadDelimited()` escape. Tests: `=SUM(A1)`, `@cmd`, `+1`, `-1` on all 3 export paths | `helpers.py`, `routes.py`, `static/js/app.js`, `tests/test_routes.py` | **F1** |
| 1.2 | **Log hygiene**: `redact_url()` (strip query/fragment/userinfo) in `security.py`; use it in the `logger.warning` in the API-fetch except; add `caplog` assertions that tokens never reach logs | `security.py`, `routes.py`, `tests/test_routes.py` | F3, F9 |
| 1.3 | **API-fetch JSONL ValueError → 400** generic message (before outer handler); ensures no response snippet is logged | `routes.py`, `tests/test_routes.py` | F9 |
...
| 1.5 | **Header hardening**: add HSTS (secure requests only), `Permissions-Policy`, `Cross-Origin-Opener-Policy`, `Cross-Origin-Resource-Policy`; extend CSP with `object-src 'none'; base-uri 'self'; frame-ancestors 'none'; form-action 'self'`; keep `X-Frame-Options: DENY` | `security.py`, `tests/test_routes.py` | F5 |
...
| 1.7 | **Recursion-depth guards**: `_depth`/`max_depth` on `extract_table_data` and `find_candidate_arrays` (mirror `flatten_for_csv`); wrap `json.loads`/`parse_jsonl` callers to catch `RecursionError` → 400 "JSON nesting too deep"; 1500-deep nesting test | `helpers.py`, `routes.py`, `tests/test_helpers.py`, `tests/test_routes.py` | F8 |
| 1.8 | **Bounded DNS**: `getaddrinfo` via `ThreadPoolExecutor` + `API_DNS_TIMEOUT` (default 3s) | `security.py`, `config.py`, `tests/test_security.py` | F6.1, P7 |
| 1.9 | **Port allowlist** for API fetch (D5): `API_ALLOWED_PORTS` default `80,443,8443` | `security.py`/`routes.py`, `config.py`, tests | F6.2 |
| 1.10 | **Proxy-aware rate limiting** (D3): optional `TRUST_PROXY=1` → `ProxyFix` in `create_app`; custom `key_func` honoring `X-Forwarded-For`; document Redis storage for multi-instance in `MEMORY.md` | `app.py`, `extensions.py`, `config.py`, tests | F12 |
...
| 1.14 | **Cookie hardening**: `SESSION_COOKIE_HTTPONLY=True`, `SAMESITE='Lax'`, `SECURE=not DEBUG` | `config.py`, tests | F16 |
| 1.15 | **Version disclosure gating** (with D4 if auth lands): `HEALTH_REVEAL_VERSION` default off | `routes.py`, `config.py` | F15 |
...
| # | Task | Files | Fixes |
|---|---|---|---|
| 2.1 | **gzip middleware** (D1): compress JSON/text bodies >1 KB when client accepts gzip; set `Content-Encoding: gzip`, `Vary: Accept-Encoding`; skip export streaming responses | `app.py` (or `security.py`), tests | P1 |
| 2.2 | **Static cache headers**: `SEND_FILE_MAX_AGE_DEFAULT=86400` + `?v=APP_VERSION` on CSS/JS URLs | `config.py`, `templates/index.html` | P6 |
| 2.3 | **Streaming exports**: `Workbook(write_only=True)` + `SpooledTemporaryFile` + `send_file`; CSV generator response; `MAX_EXPORT_ROWS` guard (default 100k) | `routes.py`, `config.py`, tests | P3 |
| 2.4 | **Preview truncation**: cap long strings / nested arrays / nested objects in `preview` rows only (full fidelity kept in `csv_data`) | `routes.py`, `helpers.py`, tests | P2.2, P5 |
...
| 2.7 | **Memory trim**: decode `bytearray` directly (drop `bytes()` copy) in API-fetch; single-pass flatten + column accumulation | `routes.py`, `helpers.py`, tests | P12, P8 |
...
**Goal:** reduce `process_json` complexity (cyclomatic ≈ 15), remove dead code, eliminate known correctness nits. Carries forward the unimplemented recommendations from `docs/code-health-final.md` §4.1/§4.3/§7.
...
| 3.3 | Remove dead code `find_candidate_arrays` + its tests (D2) | `helpers.py`, `tests/test_helpers.py` | P10 |
| 3.4 | Return `preview_limit` in `/process` payload; use it for the badge; note sort-scope in UI text | `routes.py`, `static/js/app.js` | P11 |
| 3.5 | Move `openpyxl` import to module top (fails fast on missing dep) | `routes.py` | — |
| 3.6 | Type annotations on `helpers.py`/`security.py` signatures (mypy optional; skip strict mode to limit scope) | `helpers.py`, `security.py` | — |

**Acceptance:** all existing tests pass unchanged; `process_json` ≤ 50 lines; `find_candidate_arrays` gone; badge reflects config; no behavior change visible to API consumers.
...
| 4.3 | **New client-side exports**: JSONL and Markdown table (per `MEMORY.md` guidance — text formats stay client-side) | `static/js/app.js`, `templates/index.html` | Dropdown additions; reuse F1 sanitization |
...
| # | Task | Files |
|---|---|---|
| 5.1 | Sync `MEMORY.md`: multi-array handshake entry → tree-picker handshake; add entries for formula injection, `redact_url`, ProxyFix/TRUST_PROXY, DNS timeout, gunicorn timeout invariant, port allowlist | `MEMORY.md` |
...
**Risk order:** Phase 0 first (deploying with gunicorn 21.2.0 while knowing about CVE-2024-1135 is the standing exposure), then Phase 1.1 (formula injection) before any further feature work on exports. Phases 2-4 build on the refactored `process_json` to keep the diff small.
