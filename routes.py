...
def _build_api_auth(auth_method):
    """
    Build the outbound headers/auth/params for one auth method.

    Returns (headers, auth, params, error_response). error_response is None
    unless the client asked for something we refuse to forward.
    """
    headers = {}
    auth = None
    params = {}
    ...
    return headers, auth, params, None


def _parse_payload(text, data_format):
    """Parse a payload as JSON or JSONL according to the requested format."""
    return parse_jsonl(text) if data_format == 'jsonl' else json.loads(text)


def _load_from_file(data_format):
    """Read and parse an uploaded file. Returns (data, error_response)."""
    ...
    if upload_error:
        return None, (jsonify({'error': upload_error}), 400)

    try:
        content = file.read().decode('utf-8')
        return _parse_payload(content, data_format), None
    except UnicodeDecodeError:
        return None, (jsonify({'error': 'File must be UTF-8 encoded'}), 400)
    except (json.JSONDecodeError, ValueError) as e:
        return None, (jsonify({'error': f'Invalid data in file: {e}'}), 400)


def _load_from_paste(data_format):
    """Parse pasted text. Returns (data, error_response)."""
    pasted_json = request.form.get('pasted_json', '').strip()
    if not pasted_json:
        return None, (jsonify({'error': 'No JSON provided'}), 400)

    try:
        return _parse_payload(pasted_json, data_format), None
    except (json.JSONDecodeError, ValueError) as e:
        return None, (jsonify({'error': f'Invalid data: {e}'}), 400)


def _load_from_api(data_format):
    """Fetch and parse a remote payload. Returns (data, error_response)."""
    api_url = request.form.get('api_url', '').strip()
    if not api_url:
        return None, (jsonify({'error': 'No API URL provided'}), 400)

    is_valid, error_msg = validate_url(api_url)
    if not is_valid:
        return None, (jsonify({'error': error_msg}), 400)

    headers, auth, params, error_response = _build_api_auth(request.form.get('auth_method', 'none'))
    if error_response:
    ...
    try:
        ...
        # Fixed message, no interpolation: requests' exception text carries the
        # full URL, and the query string, fragment, userinfo AND path can each
        # hold a token (F3/F9). Redacting one component is not enough, so nothing
        ...
        # parse_jsonl raises ValueError on a malformed line. Without this it
        # reached the outer handler as a 500 with a logged traceback, although it
        # is the caller's data that is wrong (F9).
        return None, (jsonify({'error': 'API response is not valid JSONL'}), 400)


def _load_input(data_format):
    """
    Dispatch to the requested input method. Returns (data, error_response).

    Exactly one of the two is meaningful: a non-None error_response is the
    caller's return value.
    """
    loaders = {
        'file': _load_from_file,
        'paste': _load_from_paste,
        'api': _load_from_api,
    }
    loader = loaders.get(request.form.get('input_method'))
    if loader is None:
        return None, (jsonify({'error': 'Invalid input method'}), 400)
    return loader(data_format)


def _select_table_data(json_data, json_path):
    """
    Turn the chosen JSON path into table rows. Returns (rows, error_response).

    With no path the client has not chosen a node yet, so the whole document
    goes back for the tree picker -- that 200 is a response, not an error, and
    rides in the error_response slot because it is equally terminal.
    """
    if not json_path:
        return None, (jsonify({'needs_selection': True, 'raw_json': json_data}), 200)

    selected = extract_by_path(json_data, json_path)
    if selected is None:
        return None, (jsonify({'error': f'Path "{json_path}" not found'}), 400)

    if isinstance(selected, list):
        rows = extract_table_data(selected, max_depth=current_app.config['FLATTEN_MAX_DEPTH'])
    elif isinstance(selected, dict):
        rows = [selected]
    else:
        return None, (
            jsonify({'error': f'Path "{json_path}" is a primitive value; pick an object or array'}),
            400,
        )

    if not rows:
        return None, (jsonify({'error': 'Could not extract tabular data from JSON'}), 400)
...
def process_json():
    ...
    try:
        ...
        json_data, error_response = _load_input(data_format)
        if error_response:
            return error_response

        table_data, error_response = _select_table_data(
            json_data, request.form.get('json_path', '')
        )
        if error_response:
            return error_response
        ...
        csv_data, csv_columns = flatten_rows(
            table_data, max_depth=current_app.config['FLATTEN_MAX_DEPTH']
        )
        ...
        # BEFORE the user clicks (D6); preview_limit drives the badge (P11).
        return jsonify(
            {
                'success': True,
                'columns': columns,
